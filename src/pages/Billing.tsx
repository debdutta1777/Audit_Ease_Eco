...
import { Check, Star, Lock, Users, Shield, Sparkles, Link2, CreditCard } from 'lucide-react';
...
export default function Billing() {
    ...
    const handleSubscribe = (planId: string) => {
        ...
        const price = plan?.monthlyPrice ? (isAnnual ? Math.round(plan.monthlyPrice * 0.8) : plan.monthlyPrice) : 0;

        if (price > 0) {
            setSelectedPlan({ id: planId, name: plan?.name || planId, price });
            setPaymentDialogOpen(true);
        }
    };

    const handlePaymentSuccess = async (paymentMethod: string, txHash?: string) => {
        if (!selectedPlan) return;
        setLoading(selectedPlan.id);
        try {
            const planTier = (selectedPlan.id === 'pro' ? 'professional' : selectedPlan.id) as 'professional' | 'enterprise';
            const method = paymentMethod === 'crypto' ? 'crypto' as const : 'card' as const;
            await upgradePlan(planTier, isAnnual ? 'annual' : 'monthly', method, txHash);

            const methodLabel = paymentMethod === 'crypto' ? 'blockchain' :
            ...
            toast.success(`Successfully upgraded to ${selectedPlan.name} plan!`, {
                description: txHash
                    ? `Paid via ${methodLabel} • TX: ${txHash.slice(0, 10)}...`
                    : `Paid via ${methodLabel} • ${isAnnual ? 'Annual' : 'Monthly'} billing`,
            });
            ...
            <div className="max-w-6xl mx-auto space-y-12 pb-16">
                ...
                <div className="mt-16 flex flex-wrap items-center justify-center gap-8 text-sm font-medium text-slate-600 dark:text-gray-400 animate-slide-up" style={{ animationDelay: '0.5s' }}>
                    {[
                        ...
                        { icon: Link2, text: 'Blockchain Payments' },
                    ].map((badge) => (
