...
import { ArrowLeft, Download, CheckCircle2, MessageSquare, FileText, Sparkles, Wand2, Eye } from 'lucide-react';
...
import { GeneratingCompliancePage } from '@/components/audit/GeneratingCompliancePage';
...
export default function AuditDetail() {
  ...
  const fetchAudit = async () => {
    ...
    if (auditData) {
      ...
      // Restore compliant contract from DB if it exists
  ...
  const handlePurchaseSuccess = async () => {
    // Close the dialog and show the generating page
    ...
    <AppLayout>
      <div className="space-y-8 h-full">
        ...
        {audit.status === 'analyzing' || audit.status === 'pending' ? (
          ...
          <>
            {/* Show generating page as full-screen overlay */}
            {isGeneratingRewrite ? (
              <GeneratingCompliancePage />
            ) : (
      ...
      <PurchaseDialog
        ...
        featureName="Premium Contract Repair"
      />
    </AppLayout>
  );
}